...
public class DispatcherServlet extends HttpServlet {
	...
	public void init() throws ServletException {
		...
		try {
			prop.load(new FileInputStream("C:/dev/lecture/1_java/workspace/xciweb01/src/main/java/mvc/fx/dispatcher-servlet.properties")); // c:/dev/dispatcher-servlet.properties
			for (Object oKey : prop.keySet()) {
				String key = ((String) oKey).trim(); // 공백처리
				Class<?> className = null;
				try {
					className = Class.forName(prop.getProperty(key).trim()); // 공백처리 
